function xdot = stochasticFourTankSystem(t,x,u,p)
% MODIFIEDFOURTANKSYSTEM Model dx/dt = f(t,x,u,d,p) for Stochastic Modified 4-tank System
%
% This function implements a differential equation model for the
% modified 4-tank system.
%
% Syntax: xdot = stochasticModifiedFourTankSystem(t,x,u,d,p)

% Unpack states, MVs, and parameters
m = x; % Mass of liquid in each tank [g]
F = u; % Flow rates [cm3/s]
...
xdot(3,1) = rho*(qin(3,1)-qout(3,1)+W(1)); % Mass balance Tank 3
xdot(4,1) = rho*(qin(4,1)-qout(4,1)+W(2)); % Mass balance Tank 4
