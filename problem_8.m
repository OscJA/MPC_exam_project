...
for i=1:(tf_-t0)/Ts
    ...
    U = constrainedOptimization(X(:, i), wk, U(1:dim_u), ulb, uub, Z_bar, Mdu, H_z, H_u, H_du, g_u, rho_u, WI, gz_mat, rhoz_mat, Phi_x, Phi_w, N);
...
figure;
plot(t0:Ts:(tf_-Ts), ys(1)+Y(1, :), '-b');
hold on;
plot(t0:Ts:tf_, Z_bars(1, 1:(tf_-t0)/Ts+1)+ys(1), '--r');
hold off;
title('Tank 1 height');

figure;
plot(t0:Ts:(tf_-Ts), ys(2)+Y(2, :), '-b');
hold on;
plot(t0:Ts:tf_, Z_bars(2, 1:(tf_-t0)/Ts+1)+ys(2), '--r');
hold off;
title('Tank 2 height');
